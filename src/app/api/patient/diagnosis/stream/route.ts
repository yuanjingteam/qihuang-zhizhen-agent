...
const diagnosisRequestSchema = z.object({
  ...
  inquiryResponses: z.array(z.string()).optional(),
});

/**
 * POST /api/patient/diagnosis/stream
 *
 * SSE streaming diagnosis for patient (2C: food-medicine advice only)
 * If inquiryResponses are provided, skips the inquiry phase and uses the provided answers.
 */
  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        // Stream diagnosis events (with optional inquiry responses)
        for await (const event of runner.runStream(patient, "2C", parsed.inquiryResponses)) {
